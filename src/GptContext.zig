...
backup_header: ?gpt.GptHeader,
partition_entries: ?[]gpt.GptEntry,
...
fn validateHeaderCrc32(header: *gpt.GptHeader) !void {
    ...
    if (original_crc != calculated_crc) {
        return gpt.GptError.InvalidCrc32;
    }
}

fn updateHeaderCrc32(header: *gpt.GptHeader) void {
    header.header_crc32 = 0;
    const crc = calculateCrc32(std.mem.asBytes(header)[0..header.getHeaderSize()]);
    header.header_crc32 = std.mem.nativeToLittle(u32, crc);
}

fn validatePartitionArrayCrc32(header: *const gpt.GptHeader, entries: []const gpt.GptEntry) !void {
...
fn updatePartitionArrayCrc32(header: *gpt.GptHeader, entries: []const gpt.GptEntry) void {
    const entries_size = header.getNumPartitions() * header.getPartitionEntrySize();
...
pub fn readPrimaryHeader(self: *Self) !void {
    ...
    try self.readSector(gpt.GPT_PRIMARY_PARTITION_TABLE_LBA, &sector_buffer);

    const header = std.mem.bytesToValue(gpt.GptHeader, &sector_buffer);

    if (!header.isValid()) {
        return gpt.GptError.InvalidSignature;
    }
...
pub fn readPartitionEntries(self: *Self) !void {
    ...
    if (entry_size != @sizeOf(gpt.GptEntry)) {
        return gpt.GptError.InvalidHeaderSize;
    }
    ...
    const entries = try self.allocator.alloc(gpt.GptEntry, num_entries);
...
pub fn writePrimaryHeader(self: *Self) !void {
    ...
    try self.writeSector(gpt.GPT_PRIMARY_PARTITION_TABLE_LBA, &sector_buffer);
...
pub fn writeBackupHeader(self: *Self) !void {
    ...
    backup_header.alternate_lba = std.mem.nativeToLittle(u64, gpt.GPT_PRIMARY_PARTITION_TABLE_LBA);
...
pub fn findPartitionByName(self: *Self, name: []const u8, allocator: Allocator) ?*gpt.GptEntry {
    if (self.partition_entries == null) return null;
